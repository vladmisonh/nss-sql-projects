...
(SELECT 
    ...
    SUM(total_claim_count) AS total_claims_sum
FROM prescriber
	LEFT JOIN prescription
		USING(npi)
WHERE specialty_description IN ('Interventional Pain Management', 'Pain Management')
...
SELECT 
    COALESCE(specialty_description, '') AS specialty_description, 
    SUM(total_claim_count) AS total_claims
FROM prescriber
	LEFT JOIN prescription
		USING(npi)
...
ORDER BY specialty_description ASC NULLS FIRST;
...
SELECT 
    COALESCE(p1.specialty_description, '') AS specialty_description, 
    COALESCE(d.opioid_drug_flag, '') AS opioid_drug_flag, 
...
ORDER BY specialty_description ASC NULLS FIRST, opioid_drug_flag ASC NULLS FIRST;
